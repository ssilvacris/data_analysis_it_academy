...
/*Tu tarea es diseñar y crear una tabla llamada "credit_card" que almacene detalles cruciales 
 las tarjetas de crédito. La nueva tabla debe ser capaz de identificar de forma única cada tarjeta 
...
-- inserir los datos en la tabla credit_card
...
-- create a constraint  foreign key

alter table transaction
add constraint fk_transation_credit_card
...
-- cambiar la columna expiring_date de varchar para date

update credit_card
set expiring_date = str_to_date(expiring_date, '%m/%d/%Y');  -- cambio para el formato que mysql acepta pero todavia es un varchar

alter table credit_card modify expiring_date date; -- cambio para el formato date
...
con ID CcU-2938. La información que debe mostrarse para este registro es: R323456312213576817699999. 
Recuerda mostrar que el cambio se realizó.*/

select * from credit_card
where id = 'CcU-2938';

update credit_card   -- actualizando la tabla
set iban = 'R323456312213576817699999'   -- cambio de la cuenta del usuario
where id = 'CcU-2938';

select *    -- comprobando el cambio
from credit_card
where id = 'CcU-2938';
...
-- antes de inserir los valores, voy a hacerlos en las tablas donde ellos son primary key: 

insert into credit_card (id)
values ( 'CcU-9999');

select * from credit_card -- comprobando el ingreso de los valores
where id = 'CcU-9999';

insert into company (id)
values ('b-9999');

select * from company -- comprobando el ingreso de los valores
where id = 'b-9999';
...
select * from transaction -- comprobando el ingreso de los valores
where id = '108B1D1D-5B23-A76C-55EF-C568E49A99DD';


/* Los valores fueron añadidos en la tabla transaction y el credit_card_id CcU-9999, el company_id b-9999 ( posteriormente el user_id 999) 
 fueron añadidos en sus respectivas tablas donde estan como primary key.
 Las otras columnas de sus respectivas tablas se quedaran vacias temporariamente hasta que el departamento de recursos humanos  informe
...
-- verificar la tabla antes del cambio
select * from credit_card;
...
select * from  credit_card; --  Verificar el cambio mostrando la estructura de la tabla.
...
Se ha solicitado crear una vista que proporcione detalles clave sobre las compañías y sus transacciones. Será necesaria 
que crees una vista llamada VistaMarketing que contenga la siguiente información: Nombre de la compañía. 
...
select c.company_name, c.phone, c.country, avg(t.amount) as media_compra_realizada
from company c 
join transaction t
on c.id = t.company_id
where t.declined = 0
group by c.company_name, c.phone, c.country
order by media_compra_realizada desc;
...
-- Insercion de los datos en la tabla user
...
SET foreign_key_checks = 1;
 ...
 -- adición de usuario "9999" a la tabla user
insert into user (id)
values ( '9999');

select * from user -- comprobando el ingreso de los valores
where id = '9999';


-- obtener el nombre de la restriccion de la tabla user con transaction 

show create table user;

-- establecer una relación 1 a N entre las tablas user y  la tabla transaction

 alter table transaction
add constraint fk_transation_user
foreign key(user_id) references user(id);

-- eliminar la restriccion de la FK de la tabla user 

alter table user
drop foreign key user_ibfk_1;


-- Cambios en la tabla user

	-- Cambio de nombre de la tabla de user para data_user
    rename table user to data_user;
	show tables;
    
    
    
	-- Cambiar la columna email para personal_email
...
-- Cambios en la tabla company
	...
	describe company; -- para verificar la columna antes de su eliminacion

	-- eliminacion de la columna website
    alter table company drop column website;

	describe company; -- comprobar la eliminacion

-- Cambios en la tabla credit_card

	describe credit_card; -- descripcion de la tabla antes de las alteraciones
    
	-- verificar el nombre de la restriccion credit_card - transaction
    show create table transaction;
    
    -- eliminar la restriccion de la FK de la tabla transaction 

	alter table transaction
	drop foreign key fk_transation_credit_card ;

    
	-- Cambio de la columna id de VARCHAR(50) para VARCHAR(20)
    
    alter table credit_card modify column id VARCHAR(20);

    
	-- Cambio de la columna pin de VARCHAR(5) para VARCHAR(4)
    
    alter table credit_card modify column pin VARCHAR(4);
    
	-- Añadir columna, fecha_actual y datatype DATE

	alter table credit_card add column fecha_actual DATE;
    
    -- cambio de la columna expiring_date de DATE  VARCHAR(10)
...
-- volver con la restriccion en la tabla transaction referente a la tabla credit_card
...
select t.id as 'Transaction ID' , u.name as 'User name' , u.surname as 'User last name', 
cc.iban as 'IBAN credit card', c.company_name as 'Company name of the transaction'
from transaction t
join data_user u
on t.user_id = u.id
join company c 
...
-- selecionar los datos del informetecnico
select * from transactions.informetecnico;

-- verificar el id agregado en el ejercicio 3, nivel 1
